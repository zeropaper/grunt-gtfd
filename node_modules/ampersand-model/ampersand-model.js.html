...
<body>
  ...
  <div class="container content">
    <div class="row"><div class="documentation"></div><div class="code"><pre><code class="js"><span class="hljs-comment">/*$AMPERSAND_VERSION*/</span>
...
<span class="hljs-keyword">var</span> Model = State.extend({
    save: <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">(key, val, options)</span> </span>{
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Handle both <code>&quot;key&quot;, value</code> and <code>{key: value}</code> -style arguments.</p>
</div><div class="code"><pre><code class="js">        <span class="hljs-keyword">if</span> (key == <span class="hljs-literal">null</span> || <span class="hljs-keyword">typeof</span> key === <span class="hljs-string">'object'</span>) {
            attrs = key;
            options = val;
...
</code></pre></div></div><div class="row"><div class="documentation"><p> If we’re not waiting and attributes exist, save acts as<br> <code>set(attr).save(null, opts)</code> with validation. Otherwise, check if<br> the model will be valid when the attributes, if any, are set.</p>
</div><div class="code"><pre><code class="js">        <span class="hljs-keyword">if</span> (attrs &amp;&amp; !options.wait) {
...
</code></pre></div></div><div class="row"><div class="documentation"><p> After a successful server-side save, the client is (optionally)<br> updated with the server-side state.</p>
</div><div class="code"><pre><code class="js">        <span class="hljs-keyword">if</span> (options.parse === <span class="hljs-keyword">void</span> <span class="hljs-number">0</span>) options.parse = <span class="hljs-literal">true</span>;
        ...
        <span class="hljs-keyword">if</span> (method === <span class="hljs-string">'patch'</span>) options.attrs = attrs;</code></pre></div></div><div class="row"><div class="documentation"><p> if we’re waiting we haven’t actually set our attributes yet so<br> we need to do make sure we send right data</p>
</div><div class="code"><pre><code class="js">        <span class="hljs-keyword">if</span> (options.wait) options.attrs = _.extend(model.serialize(), attrs);
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Fetch the model from the server. If the server’s representation of the<br> model differs from its current attributes, they will be overridden,<br> triggering a <code>&quot;change&quot;</code> event.</p>
</div><div class="code"><pre><code class="js">    fetch: <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">(options)</span> </span>{
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Destroy this model on the server if it was already persisted.<br> Optimistically removes the model from its collection, if it has one.<br> If <code>wait: true</code> is passed, waits for the server to respond before removal.</p>
</div><div class="code"><pre><code class="js">    destroy: <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">(options)</span> </span>{
        options = options ? _.clone(options) : {};
        <span class="hljs-keyword">var</span> model = <span class="hljs-keyword">this</span>;
        <span class="hljs-keyword">var</span> success = options.success;
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Proxy <code>ampersand-sync</code> by default — but override this if you need<br> custom syncing semantics for <em>this</em> particular model.</p>
</div><div class="code"><pre><code class="js">    sync: <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">()</span> </span>{
        <span class="hljs-keyword">return</span> sync.apply(<span class="hljs-keyword">this</span>, <span class="hljs-built_in">arguments</span>);
    },
</code></pre></div></div><div class="row"><div class="documentation"><p> Default URL for the model’s representation on the server — if you’re<br> using Backbone’s restful methods, override this to change the endpoint<br> that will be called.</p>
</div><div class="code"><pre><code class="js">    url: <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">()</span> </span>{
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Throw an error when a URL is needed, and none is supplied.</p>
</div><div class="code"><pre><code class="js"><span class="hljs-keyword">var</span> urlError = <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">()</span> </span>{
...
</code></pre></div></div><div class="row"><div class="documentation"><p> Wrap an optional error callback with a fallback error event.</p>
</div><div class="code"><pre><code class="js"><span class="hljs-keyword">var</span> wrapError = <span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-params">(model, options)</span> </span>{
